...
def add_event(request):
    ...
    if eid == '' or name == '' or address == '' or status == '' or start_time == '':
    ...
    try:
        ...
        error = 'start_time format error. is must be in YYYY-MM-DD HH:MM:SS format.'
...
def add_guest(request):
    ...
    result = Event.objects.filter(id=eid)
    if not result:
        return JsonResponse({'status':10022,'message':'enent id null'})
    result = Event.objects.get(id=eid).status
    if not result:
        return JsonResponse({'status':10023,
                             'message':'enent status is not anailable'})
    ...
    event_time = Event.objects.get(id=eid).start.time   #发布会时间
    etime = str(event_time).split(".")[0]
    timeArray = time.strftime("%Y-%m-%d %H:%M:%S",etime)
...
def get_guest_list(request):
    ...
    if eid != '' and phone !='':
        ...
        try:
            ...
            return JsonResponse({'status':200,"message":"sucess","data":guest})
...
def user_sign(request):
    ...
    if result:
        ...
        Guest.objects.filter(event_id=eid, phone=phone).updata(sign='1')
